...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if(indexPath.row == 2){
        ...
        //Security
        self.optionType = 4;
        optionList = @[@"WPAPSKWPA2PSK", @"WPA2PSK", @"WPAPSK", @"WEPAUTO", @"OPEN"];
...
- (void)GenericResponseCallback:(id)sender {
    ...
    if (data != nil) {
        ...
        if (isSuccessful) {
            ...
            NSData *data_copy = [obj.decodedData mutableCopy];  //[obj.genericData dataUsingEncoding:NSUTF8StringEncoding];
            [genericData appendData:data_copy];
