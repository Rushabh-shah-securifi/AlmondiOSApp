...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForSubRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    Client * connectedDevice = self.connectedDevices[indexPath.section];
    ...
    switch (subRowIndex) {
        ...
        {
            if (!((Client*)self.connectedDevices[indexPath.section]).isActive){
                UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(tblDevices.frame.size.width - 215, 0, 200, propertyRowCellHeight)];
                label.backgroundColor = [UIColor clearColor];
                label.textColor = [UIColor whiteColor];
                label.alpha = 0.5;
...
-  (void)addCellLabel:(UITableViewCell*)cell IndexPath:(NSIndexPath *)indexPath connectDevice:(Client*)connectedDevice{
    NSInteger subRowIndex = indexPath.subRow-1;
    BOOL local=[self getLocalConnection];
    if(local && (subRowIndex==notifyMeIndexPathRow || subRowIndex == historyIndexPathRow))
        return;
    
    if (!((Client*)self.connectedDevices[indexPath.section]).deviceUseAsPresence && subRowIndex==historyIndexPathRow) {
        return;
    }
    if (((Client*)self.connectedDevices[indexPath.section]).isActive  && subRowIndex==lastActiveTimeIndexPathRow) {
        return;
    }
    ...
    switch (subRowIndex) {
        ...
        case notifyMeIndexPathRow://Notify me
        {
            if(!local){
                ...
                viewController.selectedNotificationType = [self getNotificationTypeForDevice:((Client*) self.connectedDevices[indexPath.section]).deviceID];
                viewController.delegate = self;
                viewController.editFieldIndex = subRowIndex;
                viewController.connectedDevice = self.connectedDevices[indexPath.section];
                [self.navigationController pushViewController:viewController animated:YES];
...
- (void)updatePreferenceInfo:(NSDictionary *)preferenceInfo{
    ...
    for (Client * device in self.connectedDevices) {
