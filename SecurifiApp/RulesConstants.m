//
//  RulesConstants.m
//  Tableviewcellpratic
//
//  Created by Masood on 18/11/15.
//  Copyright © 2015 Securifi-Mac2. All rights reserved.
//

#import "RulesConstants.h"

@implementation RulesConstants

const int textHeight = 20;
const int frameSize = 70;
const int indexButtonFrameSize = 90;
const int dimFrameHeight = 75;
const int dimFrameWidth = 140;
const int textPadding = 2;
const int fontSize = 11;
const int topFontSize = 12;
float const imageHeightFactor = 1.6;

const int hueSubViewSize = 20;
const int ROW_PADDING = 25;

const int entryBtnWidth = 52;
const int entryBtnHeight = 90;
const int separatorWidth=20;
const int entryPadding=5;
const int countDiameter = 16;
const int TOPSCROLLPADDING = 5;
const int rulesButtonsViewWidth = 105;
const int rulesButtonsViewHeight = 97;
const int crossButtonScale = 5.5;
@end
