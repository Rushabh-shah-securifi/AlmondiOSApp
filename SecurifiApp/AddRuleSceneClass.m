...
- (void)buildTriggersAndActions{
    [SFISubPropertyBuilder createEntryForView:self.triggersActionsScrollView indexScrollView:self.deviceIndexScrollView parentView:self.parentView parentClass:self triggers:self.triggers actions:self.actions isCrossButtonHidden:NO isRuleActive:YES];
}
...
-(void)updateInfoLabel{
    if(self.isScene){
        ...
        if(self.triggers.count == 0){
            ...
            self.informationLabel.text = @"Add another arigger/action or press SAVE to finalize the rule";
        }
    }
}
